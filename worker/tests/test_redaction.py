...
def test_endpoint_nested_credentials_and_env_secrets_are_removed(monkeypatch):
    ...
    original = {
        'body': {
            'response': 'internal-known-secret provider-key-1234 Authorization: Bearer unknown-token',
            'api_key': 'another-secret',
        }
    }
    scrubbed = redact(original, values)
    assert scrubbed['body']['api_key'] == '[REDACTED]'
    for secret in ['internal-known-secret', 'provider-key-1234', 'unknown-token', 'another-secret']:
