...
const child=spawn(process.execPath,['node_modules/next/dist/bin/next','start','--port','3187','--hostname','127.0.0.1'],{
 env:{...process.env,BENCHRX_ADMIN_TOKEN:token,BENCHRX_ADAPTER_SECRET:randomBytes(32).toString('hex'),BENCHRX_APP_ORIGIN:'https://benchrx.example',SUPABASE_SERVICE_ROLE_KEY:'',NEXT_PUBLIC_SUPABASE_URL:'',NEXT_TELEMETRY_DISABLED:'1'},stdio:['ignore','pipe','pipe']});
...
try {
 ...
 assert.equal(publicConnection.status,400,'Public connection testing no longer requires operator auth');
 ...
 const crossSite=await fetch(origin+'/api/agents/fixture/rerun',{method:'POST',headers:{Authorization:'Basic '+Buffer.from('benchrx:'+token).toString('base64'),Origin:'https://evil.example'}});
 ...
 const malformed=await fetch(origin+'/api/connections/test',{method:'POST',headers:{'Content-Type':'application/json'},body:marker});
...
console.log('Production HTTP access/CSRF checks and public malformed-JSON logging regression passed');
