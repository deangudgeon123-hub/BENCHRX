            ...
            <div className={`mt-10 flex flex-col gap-4 rounded-3xl p-5 sm:flex-row sm:items-center sm:justify-between sm:p-6 ${!positiveReadiness ? "border border-amber-500/20 bg-amber-500/10" : "border border-emerald-500/20 bg-emerald-500/10"}`}>
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <p className={`mt-1 text-sm leading-6 ${!positiveReadiness ? "text-amber-100/70" : "text-emerald-100/70"}`}>{a2aCompatibility ? "Compatibility-only interoperability checks. This does not establish BENCHRX production readiness." : productionScore === null ? `Insufficient behavioural coverage: ${insufficientCategories.join("; ")}.` : run.readiness_reasons?.length ? run.readiness_reasons.join("; ") : readinessLabel(run, productionScore)}</p>
                </div>
              </div>
