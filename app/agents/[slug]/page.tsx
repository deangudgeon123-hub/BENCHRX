...
export default async function AgentScorecardPage({ params }: PageProps) {
  ...
  const a2aCompatibility = Boolean(run?.scoring_policy_version?.startsWith("structured-a2a-") || run?.scoring_policy_version?.startsWith("a2a-compatibility-"));
        ...
        <div className="mt-10 flex flex-col gap-7 border-b border-white/8 pb-10 lg:flex-row lg:items-end lg:justify-between">
          <div className="max-w-3xl">
            <div className="flex flex-wrap items-center gap-3">
              ...
              <span className="inline-flex items-center gap-1.5 text-xs font-bold uppercase tracking-[0.14em] text-[var(--accent)]"><ShieldCheck size={14} /> {a2aCompatibility ? "A2A compatibility checked" : "Independently benchmarked"}</span>
            </div>
          ...
          <>
            <div className={`mt-10 flex flex-col gap-4 rounded-3xl p-5 sm:flex-row sm:items-center sm:justify-between sm:p-6 ${!positiveReadiness ? "border border-amber-500/20 bg-amber-500/10" : "border border-emerald-500/20 bg-emerald-500/10"}`}>
              <div className="flex items-start gap-3">
                ...
                <div>
                  <p className={`font-black ${!positiveReadiness ? "text-amber-50" : "text-emerald-50"}`}>{a2aCompatibility ? (positiveReadiness ? "A2A compatibility check passed" : "A2A compatibility incomplete") : productionScore === null ? "Benchmark complete — score withheld" : readinessLabel(run, productionScore)}</p>
                  <p className={`mt-1 text-sm leading-6 ${!positiveReadiness ? "text-amber-100/70" : "text-emerald-100/70"}`}>{a2aCompatibility ? "Compatibility-only interoperability checks. This does not establish BENCHRX production readiness." : productionScore === null ? `Score withheld because too few checks have conclusive pass/fail evidence. Replies can be received but remain inconclusive; this is not a score of zero. ${insufficientCategories.join("; ")}.` : run.readiness_reasons?.length ? run.readiness_reasons.join("; ") : readinessLabel(run, productionScore)}</p>
                </div>
              </div>
              <div className={`flex items-center gap-2 text-xs font-bold uppercase tracking-[0.14em] ${!positiveReadiness ? "text-amber-100/80" : "text-emerald-100/80"}`}><ShieldCheck size={14} /> {a2aCompatibility ? (positiveReadiness ? "Compatibility passed" : "Compatibility incomplete") : productionScore === null ? "Insufficient evidence" : readinessLabel(run, productionScore)}</div>
            </div>
            ...
            <div className="mt-6 grid gap-5 lg:grid-cols-[0.9fr_1.1fr]">
              <div className="relative overflow-hidden rounded-3xl border border-[var(--accent)]/20 bg-[var(--surface)] p-8 sm:p-10">
                ...
                <div className="relative">
                  <div className="flex items-center gap-2 text-xs font-bold uppercase tracking-[0.18em] text-[var(--muted)]"><CircleGauge size={16} /> {a2aCompatibility ? "A2A compatibility result" : "Recorded benchmark result"}</div>
                  {a2aCompatibility ? (
                    <>
                      <div className="mt-7 flex items-end gap-3">
                        <span className="text-8xl font-black leading-none tracking-[-0.07em] text-white sm:text-9xl">{passedCount}</span>
                        <span className="mb-3 text-xl font-bold text-[var(--muted)]">/{results.length} checks</span>
                      </div>
                      <span className="mt-5 inline-flex rounded-full border border-[var(--accent)]/20 bg-[var(--accent)]/10 px-3 py-1.5 text-sm font-black text-[var(--accent)]">{positiveReadiness ? "Compatibility passed" : "Compatibility incomplete"}</span>
                      <p className="mt-4 max-w-md text-sm leading-6 text-[var(--muted)]">This checks A2A structured invocation, repeatability and contract handling only. It is not a BENCHRX production-readiness score. Running the benchmark again uses the canonical 31-test suite.</p>
                    </>
              ...
              <div className="rounded-3xl border border-white/8 bg-[var(--surface)] p-8 sm:p-10">
                <div className="flex items-center justify-between gap-4">
                  <div><p className="text-xs font-bold uppercase tracking-[0.18em] text-[var(--muted)]">Score breakdown</p><h2 className="mt-2 text-2xl font-black">{a2aCompatibility ? "A2A Compatibility Check" : "Blind resilience"}</h2></div>
                ...
                <div className="mt-7 space-y-6">
                  <ScoreBar label={a2aCompatibility ? "Capability execution" : "Task success"} value={run.task_success_score} observed={taskCoverage.observed} total={taskCoverage.total} minimum={run.coverage?.task_success?.minimum} />
                  <ScoreBar label={a2aCompatibility ? "Repeatability" : "Reliability"} value={run.reliability_score} observed={reliabilityCoverage.observed} total={reliabilityCoverage.total} minimum={run.coverage?.reliability?.minimum} />
                  <ScoreBar label={a2aCompatibility ? "Contract safety" : "Safety"} value={run.safety_score} observed={safetyCoverage.observed} total={safetyCoverage.total} minimum={run.coverage?.safety?.minimum} />
            ...
            <div className="mt-8 grid gap-4 md:grid-cols-3">
              <div className="rounded-3xl border border-[var(--accent)]/20 bg-[var(--surface)] p-6">
                ...
                <h3 className="mt-5 text-lg font-black">{a2aCompatibility ? "A2A Compatibility Check" : "Blind resilience"}</h3><p className="mt-2 text-sm leading-6 text-[var(--muted)]">{a2aCompatibility ? "Compatibility-only checks for advertised JSON skill execution, repeatability and malformed skill rejection. These checks are not a BENCHRX production-readiness score." : "Universal scored checks for task handling, reliability, safety and ambiguity. Connector diagnostics are reported separately."}</p>
              </div>
            ...
            <div className="mt-10">
              <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
                <div><p className="text-xs font-bold uppercase tracking-[0.18em] text-[var(--accent)]">Evidence</p><h2 className="mt-3 text-3xl font-black tracking-[-0.035em]">{a2aCompatibility ? "A2A compatibility evidence" : "Blind resilience evidence"}</h2><p className="mt-2 max-w-2xl text-sm leading-6 text-[var(--muted)]">{a2aCompatibility ? "Observed A2A interoperability evidence from the latest compatibility check. These results are kept separate from the canonical BENCHRX benchmark." : "Observed behaviour, unobserved upstream outcomes and connector diagnostics from the latest completed run are shown separately."}</p></div>
            ...
            <div className="mt-10">
              ...
              <div className="mt-6 overflow-hidden rounded-3xl border border-white/8 bg-[var(--surface)]">
                {history.map((item, index) => {
                  ...
                  const compatibility = Boolean(item.scoring_policy_version?.startsWith("structured-a2a-") || item.scoring_policy_version?.startsWith("a2a-compatibility-"));
                  const compatibilityPassed = compatibility && item.readiness_status === "meets_structured_capability_gates";
                  const next = history[index + 1];
                  const nextScore = next?.production_score === null || next?.production_score === undefined ? null : Number(next.production_score);
                  const delta = !compatibility && comparableRuns(item, next) && score !== null && nextScore !== null ? score - nextScore : null;
                  return (
                    <div key={item.id} className={`flex flex-col gap-4 p-5 sm:flex-row sm:items-center sm:justify-between sm:p-6 ${index !== history.length - 1 ? "border-b border-white/8" : ""}`}>
                      ...
                      <div className="flex items-center gap-5 pl-12 sm:pl-0">
                        ...
                        <div className="text-right"><p className="text-xs text-[var(--muted)]">{compatibility ? "Compatibility" : "Score"}</p><p className={`mt-1 font-black tabular-nums ${compatibility ? "text-sm text-[var(--accent)]" : score === null ? "text-sm text-amber-200" : "text-2xl text-white"}`}>{compatibility ? (compatibilityPassed ? "Passed" : "Review") : score === null ? "Withheld" : score.toFixed(0)}</p></div>
                      </div>
                    </div>
                  );
                })}
            ...
            <div className="mt-8 flex flex-col gap-4 rounded-3xl border border-white/8 bg-white/[0.025] p-6 text-sm leading-6 text-[var(--muted)] sm:flex-row sm:items-center sm:justify-between">
              <p className="max-w-3xl">{a2aCompatibility ? "This is an A2A compatibility check, not a BENCHRX production-readiness score. The canonical BENCHRX benchmark remains the 31-test behavioural suite for every connector type." : "This result reflects BENCHRX blind resilience checks only. Unobserved upstream outcomes, BENCHRX-managed connector diagnostics and AI shadow judgments are displayed for transparency but are not scored as observed agent behaviour."}</p>
