...
function defaultMatchesType(value: unknown, schema: Record<string, unknown>, depth = 0): boolean {
  if (depth > 4) return false;
  if (Array.isArray(schema.anyOf)) return schema.anyOf.length <= 16 && schema.anyOf.some(raw => defaultMatchesType(value, object(raw), depth + 1));
  if (Array.isArray(schema.enum) && !schema.enum.includes(value)) return false;
  if (Array.isArray(schema.type)) return schema.type.length <= 8 && schema.type.some(type => defaultMatchesType(value, {...schema, type}, depth + 1));
  switch (schema.type) {
    ...
    default: return true; // Missing schema type does not invalidate an existing declared default.
  }
}
...
export function assistantOutputIndex(e: GradioEndpoint): number {
  ...
  // object/json schemas, or a JSON component whose public schema omits the value type.
  // Only neutral/unset or JSON components are eligible; invocation still has to extract
  // assistant-authored text before the connection succeeds.
  const structured = e.outputs.map((p, i) => ({p, i})).filter(({p}) =>
    (!p.component || p.component === 'json') &&
    (/^(array|list|object|json)$/.test(p.type) || (!p.type && p.component === 'json')) &&
