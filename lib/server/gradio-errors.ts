...
export class GradioInvocationError extends Error {
  ...
  code: 'timeout' | 'response_limit' | 'serialization' | 'transport' | 'http_status' | 'invalid_event_id' | 'job_failed' | 'rate_limited' | 'gpu_unavailable' | 'invalid_input' | 'upstream_runtime_error' | 'incomplete_stream' | 'invalid_json' | 'invalid_output';
  httpStatus?: number;
  stepIndex?: number;
